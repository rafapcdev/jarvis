...
// Aqui passamos a URL base da Groq, que emula a OpenAI perfeitamente e de
// graça!
...
void setup() {
  ...
  pinMode(EXT_BUTTON_PIN,
          INPUT_PULLUP); // Botão externo (pull-up interno = sem resistor)
  ...
  // Inicia o Microfone
  gptChat.initializeRecording(I2S_MIC_SERIAL_CLOCK, I2S_MIC_LEFT_RIGHT_CLOCK,
                              I2S_MIC_SERIAL_DATA, SAMPLE_RATE, I2S_MODE_STD,
                              I2S_DATA_BIT_WIDTH_16BIT, I2S_SLOT_MODE_MONO,
                              I2S_STD_SLOT_LEFT);

  Serial.println("[ESP1] Pronto! Segure o botao BOOT (GPIO 0) ou o botao "
                 "externo (GPIO 4) para falar.");
}

void loop() {
  // Qualquer um dos dois botões ativa a gravação
  buttonPressed = (digitalRead(BOOT_BUTTON_PIN) == LOW) ||
                  (digitalRead(EXT_BUTTON_PIN) == LOW);
  ...
  if (buttonPressed && !wasButtonPressed && !gptChat.isRecording()) {
    ...
    if (textoTranscrito.length() > 0) {
      ...
      esp_now_send(enderecoESP2, (uint8_t *)textoTranscrito.c_str(),
                   textoTranscrito.length() + 1);
    } else {
  ...
  if (Serial.available() > 0) {
    ...
    if (entradaSerial.length() > 0) {
      ...
      esp_now_send(enderecoESP2, (uint8_t *)entradaSerial.c_str(),
                   entradaSerial.length() + 1);
    }
  }
